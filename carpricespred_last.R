...
library(readxl)
car_price_prediction <- read_excel("C:/Users/DELL/Downloads/car_price_prediction.xlsx")
...
mean_val <- mean(cp$Price)
median_val <- median(cp$Price)
max_val <- max(cp$Price)
min_val <- min(cp$Price)
...
mean_val <- mean(cp$Mileage)
median_val <- median(cp$Mileage)
max_val <- max(cp$Mileage)
min_val <- min(cp$Mileage)
